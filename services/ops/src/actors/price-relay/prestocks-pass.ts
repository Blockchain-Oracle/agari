 ...
 */
import { PRE_IPO_TICKERS, type TickerSymbol } from "@agari/core/market";
import { keypairSigner } from "@agari/markets/ops";
import { copyOpenSlot, PRESTOCKS_MAX_LATE_SEC, preStocksFeedHex, recordAttestedSlot, SWITCHBOARD_ERROR, type PrintSlot, type SlotOutcome } from "@agari/markets/ops/prints";
import { currentPreStocksSpot, type PreStocksSample } from "../../prices/prestocks-spot";
...
const SYMBOL_BY_FEED = new Map<string, TickerSymbol>(PRE_IPO_TICKERS.map((symbol) => [preStocksFeedHex(symbol), symbol]));

/** An attested slot on a PreStocks feed, whatever its Series' basis: the feed id, not the lane, names the source. */
export const isPreStocksSlot = (slot: PrintSlot): boolean => slot.source === "attested" && SYMBOL_BY_FEED.has(slot.feedIdHex);
...
export async function prestocksPass(ctx: RelayContext, slots: readonly PrintSlot[], chainNow: number): Promise<LanePassResult> {
  ...
  for (const slot of slots) {
    const symbol = SYMBOL_BY_FEED.get(slot.feedIdHex)!;
    const key = slotKey(slot);
    const attempts = ctx.failures.get(key)?.attempts ?? 0;
    if (attempts >= MAX_ATTEMPTS) continue;
    const chosen = chooseSample(feed.history(symbol), slot, wall);
    ...
    if (chainNow < sample.fetchedAtSec) {
      wake(sample.fetchedAtSec + 1);
      parts.push(`${short(slot)}: chain clock ${sample.fetchedAtSec - chainNow}s behind the read`);
      continue;
    }
    if (ctx.dryRun) {
      parts.push(`DRY attested ${short(slot)} @ ${sample.tokenPriceE8}e-8 read T+${sample.fetchedAtSec - slot.boundarySec}s`);
      continue;
    }
    ...
    outcome ??= await recordAttestedSlot(ctx.client, slot, {
      ...
      priceE8: sample.tokenPriceE8,
      fetchedAtSec: sample.fetchedAtSec,
    });
