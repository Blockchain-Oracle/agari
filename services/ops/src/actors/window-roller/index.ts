...
import { DEFAULT_GAP_LEAD_SEC, DEFAULT_LEAD_SEC, DEFAULT_MIN_TRADABLE_SEC } from "./plan";
...
export async function startWindowRoller(deps: VenueDeps): Promise<{ stop: () => void }> {
  ...
  deps.log(`roller ${client.payer.address}, lead ${settings.leadSec} s, gap lead ${settings.gapLeadSec} s, min tradable ${settings.minTradableSec} s${settings.only.length ? `, only ${settings.only.join(",")}` : ""}`);
