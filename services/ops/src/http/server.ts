/** The ops HTTP server (venue-ops.md §2.3, §6.5): `/health`, `/prices/latest`, `/prices/stream`. GET only, CORS `*`. */
...
export function startOpsHttp(input: { port: number; spot: SpotFeed | null; env?: OpsEnv; log?: (why: string) => void }): Promise<OpsHttp> {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(input.port, () => {
      input.log?.(`http on :${input.port} (/health, /prices/latest, /prices/stream)`);
