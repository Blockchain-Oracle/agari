//! The desk's one seam to agari-events: the shared resolve, the book read, the IOC a slot mints with, and the partial
//! redeem, all signed by the desk's own seat PDA.
//!
...
//! so nothing here measures a balance before and after to find out. `agari-leverage` carries the same file; each
...
/// The engine accounts an instruction passes, plus the desk's seat and custody.
...
/// What the desk needs from a resolved Window, copied out of engine state.
...
fn unknown<E>(_: E) -> Error {
    error!(PrivateError::UnknownMarket)
}

/// The bound Window, the engine's collateral, and the desk's own PROGRAM seat.
///
/// The seat lookup is the security of the whole desk: the engine only lets a registered `program_authorities` entry
/// hold a PROGRAM seat, so a look-alike account cannot stand in for the desk and spend its custody.
pub fn resolve(e: &Engine, collateral_mint: &Pubkey) -> Result<Window> {
    ...
    require_keys_eq!(market.series, e.series.key(), PrivateError::UnknownMarket);
    let ledger = load_checked::<Ledger>(&e.ledger, engine).map_err(unknown)?;
    require!(bound_both_ways(&market_key, &market.ledger, &e.ledger.key(), &ledger.market), PrivateError::UnknownMarket);
    if let Some(book_info) = &e.book {
        let book = load_checked::<Book>(book_info, engine).map_err(unknown)?;
        require!(bound_both_ways(&market_key, &market.book, &book_info.key(), &book.market), PrivateError::UnknownMarket);
    }
    require_keys_eq!(market.mvault, e.mvault.key(), PrivateError::UnknownMarket);

    let config = load_checked::<GlobalConfig>(&e.config, engine).map_err(unknown)?;
    require_keys_eq!(config.collateral_mint, *collateral_mint, PrivateError::WrongCollateral);

    let seat_key = e.seat.key();
    let index = config.program_authorities.iter().position(|k| *k == seat_key).ok_or(PrivateError::DeskNotRegistered)?;
    require!(index < usize::from(ledger.capacity), PrivateError::WindowPredatesDesk);
    let offset = SEAT_LEN.checked_mul(index).and_then(|o| o.checked_add(8 + LEDGER_HEADER_LEN)).ok_or(PrivateError::MathOverflow)?;
    let seat = load_slice_checked::<Seat>(&e.ledger, offset, 1).map_err(|_| error!(PrivateError::WindowPredatesDesk))?;
    require!(seat[0].owner == seat_key && seat[0].is_program(), PrivateError::WindowPredatesDesk);

    Ok(Window {
        seat_index: u16::try_from(index).map_err(|_| PrivateError::MathOverflow)?,
...
/// The side of the book a slot's mint takes, in the venue's YES terms and the arithmetic's raw units: buying YES takes
/// the asks, buying NO the bids. `(ticks × tick_base, lots × lot_base)`, best first. Every live order counts: the desk
/// spends the owner's own stake under the owner's own guard, so there is no reserve for a fresh order to mislead.
pub fn read_entry_levels(e: &Engine, w: &Window, outcome: u8, now: i64, slot: u64) -> Result<Vec<YesLevel>> {
    let book_info = e.book.as_ref().ok_or(PrivateError::UnknownMarket)?;
    let book = load_checked::<Book>(book_info, &agari_events::ID).map_err(unknown)?;
    let capacity = usize::try_from(book.capacity).map_err(|_| PrivateError::MathOverflow)?;
...
pub fn yes_ticks(limit_yes_raw: u128, w: &Window) -> Result<u16> {
    let ticks = u16::try_from(limit_yes_raw / u128::from(w.tick_base.max(1))).map_err(|_| PrivateError::MathOverflow)?;
    require!(is_valid_price(ticks), PrivateError::NothingFilled);
    Ok(ticks)
}

/// The IOC a slot mints with. Unspent escrow goes straight back to custody.
...
pub fn place(e: &Engine, seat_bump: u8, args: PlaceOrderArgs) -> Result<PlaceResult> {
    let accounts = agari_events::cpi::accounts::UserPlaceOrder {
        ...
        book: e.book.clone().ok_or(PrivateError::UnknownMarket)?,
    ...
    PlaceResult::from_return_data(&agari_events::ID, get_return_data()).ok_or_else(|| error!(PrivateError::EngineResultMissing))
}

/// `user_redeem(seat, Some(outcome), Some(lots))`: a PROGRAM partial redeem paid into custody. The seat holds every
/// slot's contracts pooled, so a settlement redeems exactly one slot's lots and leaves the rest.
