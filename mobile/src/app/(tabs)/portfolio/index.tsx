  ...
  return <TabScreen><ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.body}>
    {!address ? <View style={styles.empty}>
      ...
      <View style={[styles.balance, { backgroundColor: color.cream, borderColor: color.creamHairline }]}>
        ...
        <Text style={[TYPE.caption, { color: color.creamInk }]}>tUSDC available to call</Text>
        <View style={[styles.balanceRows, { borderTopColor: color.creamHairline }]}>
          ...
          <BalanceRow label="Locked in orders" amount={sheet ? money(sheet.orderEscrowBase) : "—"} />
          <BalanceRow label="Venue credit" amount={sheet ? money(sheet.venueCreditBase) : "—"} />
        </View>
...
function PositionRow({ position }: { position: OpenPosition }) { const { color } = useTheme(); const side = position.balanceUpRaw > position.balanceDownRaw ? "UP" : "DOWN"; return <Pressable onPress={() => router.push({ pathname: "/markets/[id]", params: { id: position.marketId } })} accessibilityRole="link" style={[styles.row, { backgroundColor: color.surface1, borderColor: color.hairline }]}><AssetDisc asset={position.asset} size={32} /><View style={styles.rowMain}><Text style={[TYPE.bodyStrong, { color: color.ink }]}>{position.asset} · {side}</Text><Text style={[TYPE.caption, { color: color.inkMuted }]}>{position.intervalSec / 60}m Window</Text></View><Text style={[TYPE.data, { color: position.unrealizedPnlBase >= 0n ? color.profit : color.loss }]}>{formatBaseUnits(position.unrealizedPnlBase, position.decimals, { signed: true })}</Text></Pressable>; }
