 ...
 */
export async function statusGate(market: EventMarket, nowMs: number): Promise<GateResult> {
  ...
  if (onchain.status !== ONCHAIN_STATUS.Trading) throw notTrading(`on-chain status ${onchain.status} is not Trading (${ONCHAIN_STATUS.Trading})`);
