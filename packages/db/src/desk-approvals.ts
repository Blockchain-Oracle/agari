...
const toApproval = (r: RawApproval): ApprovalRow => ({
  id: r.id,
  deskId: r.desk_id,
...
export function deskApprovalQueries(db: Db) {
  ...
  return {
    ...
    async listApprovals(i: { deskId: string; open?: boolean; nowSec?: number }): Promise<ApprovalRow[]> {
      ...
      const rows = i.open
        ? await db<RawApproval[]>`SELECT * FROM desk_approvals WHERE desk_id = ${i.deskId}::uuid AND answer IS NULL AND expires_at_sec > ${nowSec} ORDER BY asked_at_sec DESC`
        : await db<RawApproval[]>`SELECT * FROM desk_approvals WHERE desk_id = ${i.deskId}::uuid ORDER BY asked_at_sec DESC LIMIT 50`;
    ...
    async answerApproval(i: { deskId: string; approvalId: string; answer: "approved" | "declined"; signer: string; signature: string; nowSec: number }): Promise<{ ok: true } | { ok: false; reason: "not_found" | "already_answered" | "expired" }> {
      await ready();
      const rows = await db<RawApproval[]>`SELECT * FROM desk_approvals WHERE id = ${i.approvalId}::uuid AND desk_id = ${i.deskId}::uuid`;
      const row = rows[0];
    ...
    async approvedRequests(deskId: string): Promise<ApprovalRow[]> {
      await ready();
      const rows = await db<RawApproval[]>`SELECT * FROM desk_approvals WHERE desk_id = ${deskId}::uuid AND answer = 'approved' AND executed_seq IS NULL ORDER BY answered_at_sec ASC`;
      return rows.map(toApproval);
    },
    ...
    async expireApprovals(i: { deskId: string; nowSec: number }): Promise<ApprovalRow[]> {
      await ready();
      const rows = await db<RawApproval[]>`UPDATE desk_approvals SET answer = 'expired', answered_at_sec = ${i.nowSec} WHERE desk_id = ${i.deskId}::uuid AND answer IS NULL AND expires_at_sec <= ${i.nowSec} RETURNING *`;
      return rows.map(toApproval);
    },
     ...
     * every ten minutes per desk, counted from the last request whether or not it has run yet.
     */
    async requestCheckNow(i: { deskId: string; signer: string; signature: string; nowSec: number }): Promise<{ ok: true } | { ok: false; throttledUntilSec: number }> {
      await ready();
      return db.begin(async (tx) => {
        await tx`SELECT pg_advisory_xact_lock(hashtext(${i.deskId}))`;
        const [last] = await tx<{ scheduled_for_sec: string }[]>`SELECT scheduled_for_sec FROM desk_wakes WHERE desk_id = ${i.deskId}::uuid AND trigger = 'check_now' ORDER BY scheduled_for_sec DESC LIMIT 1`;
        ...
        await tx`INSERT INTO desk_wakes (desk_id, scheduled_for_sec, trigger, status, requested_by, signature) VALUES (${i.deskId}::uuid, ${i.nowSec}, 'check_now', 'requested', ${storageKey(i.signer)}, ${i.signature})`;
        await tx`INSERT INTO desk_events (desk_id, kind, actor, detail, at_sec) VALUES (${i.deskId}::uuid, 'check_now', 'owner', ${tx.json({})}, ${i.nowSec})`;
