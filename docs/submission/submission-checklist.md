...
| --- | --- | --- |
| **PreStocks** | Agari runs Up/Down prediction markets **on** pre-IPO token prices. It does not trade the token. `PRE-OPENAI-5m` is a real Series whose Windows open, print at both boundaries from the PreStocks catalogue and settle, with the venue attesting each price (`D-100`). Two wallets have taken opposite sides of one and been paid: a NO rest at 40¢ crossed by a YES buy at 65¢, settled DOWN against the caller, both seats redeemed and the vault drained to zero. Any of the eight PreStocks names can be listed without a program change. **Baskets** (`D-124`): five named groups of PreStocks names (AI Labs, Frontier AI, Prediction Markets, Defense & Space, All PreStocks) each run a 24/7 Up/Down Window on an equal-weight index the venue computes from one catalogue read of every member and signs like a name; a holder of two or more members can cover them together with one Down bet, and `/baskets` shows the five with their live index and Window. | The four devnet transactions in the README's Pre-IPO rows; the drive is `scripts/drive/prestocks-attest.ts`, the price module `packages/markets/src/prices/prestocks.ts`. |
| **Pyth** | Pyth is a settlement-grade source in the venue, not a display feed: TSLA/QQQ/VOO Windows resolve against a Wormhole-verified Pyth pull-oracle update admitted on-chain by `public_record_print_pyth`, cross-checked against RedStone, and a divergence voids the Window rather than paying out. Pyth's pre-IPO valuation indices (OpenAI, Anthropic) are wired on the same path as valuation lanes and as the hub's "Token vs Pyth" figure, gated on a live per-feed entitlement probe: the trial key answers `403 pyth-indices` today, so they stay unlisted and the pages omit the rows rather than show a dead lane (D-125). | The settlement rows in the README's "Settlement & claims" group, including a cross-checked settle and a `MissingPrint` void; the `/status` "Pyth valuation index · entitlement" row and `/dev/pyth-index`. |
