...
Every deferred feature gets built in full, with the full money gate, to the same quality as the rest. Done so far: S11 Blinks, S10b Range + Moonshot, S8 Earn, S10a Parlay, S9 agents and strategies, S10c Boost. **Remaining, all to be built: S10d Private desk (`agari-private`), S12b Duel arena (the arena program + `game-room` and `duel-settler`), and S12a's off-chain games where they still run on stubs.** Order them by dependency, not by what "fits": Private desk, then the arena. Start the next one without asking.
