...
A practice desk is a database row with a paper ledger `{ cashE6, positions: { [symbol]: raw } }` and no on-chain account. Every check runs the full pipeline on real prices and real Jupiter quotes at the desk's size; a "would have acted" moves the ledger with `applyPaperFill(ledger, fill)`: a buy takes `amountIn` USDC off cash and adds `quoteOut × (10000 − feeBps) / 10000` raw tokens; a sell takes `amountIn` raw tokens off the position (the gross amount leaves) and adds `quoteOut × (10000 − feeBps) / 10000` USDC. `feeBps` is `paperFeeBpsFor(routeLabels)`: C6.E measured, on the Surfpool mainnet fork, that whether Jupiter's `outAmount` nets PreStocks' fee depends on the venue. A Meteora DLMM route (Anthropic, $400) credited the desk exactly the quoted 0.378220907 raw with 0.003820414 withheld on top as `transferFeeAmount`, so its quote NETS the fee (0 bps off); a Manifest route (OpenAI, $400) quoted 0.219387119 and credited 0.217193247, 99.0 %, so its quote is GROSS (100 bps off). `FEE_NETTING_VENUES` lists the measured netting venues; anything else is taken as gross. Valuation, needs, the gate and the grade run on the ledger exactly as on a live desk. Six practice checks must exist before **Go live** unlocks (C5).
...
## 10. Mainnet runbook (C7), as rehearsed on the fork (C6)

Everything below ran on a Surfpool fork of mainnet on 2026-09-22 (`scripts/drive/desk-rehearsal.ts`, 31/31 checks; rows in `acceptance.md` under "Surfpool mainnet fork"). The real deploy repeats it with the mainnet endpoint and the user's funding (plan §9): deployer ≈ 4 SOL (the program's rent was **3.371 SOL** for the 484,208-byte `.so`, plus a write buffer the deploy refunds), init **0.020 SOL** (config 0.003007 + 8 references × 0.002116), the owner's desk **$50–100 USDC + ≈ 0.05 SOL**.

1. **Keys.** `AGARI_KEYS_DIR=~/.config/agari/mainnet pnpm roles` (ensure-style; the deployer, `desk-runner` and `price-attestor` already exist there). The program keypair stays `~/.config/agari/devnet/agari-desk-program.json` (id `4gAfiRauANHVajpErqCey7iWGbKkyuSsnXLRbApVaWak`, the same on every cluster). The mainnet attestor is a **different** key from the devnet one: the band and the premium ceiling defend a stolen operator key only while the attestor secret is kept apart (D-126).
2. **Build and the arch check.** `pnpm anchor:build` (`--arch v0`). `sha256sum anchor/target/deploy/agari_desk.so` must read `27e469605371e4819faa563c01d292149c237a1fe1eb27623fa59563233f19a0` (the fork's, 484,208 B). Check `solana feature status -u mainnet-beta | grep -i sbpf` before deploying: v0 until SBPFv3 activates (D-024).
3. **Deploy.** `solana program deploy --url mainnet-beta --use-rpc --keypair ~/.config/agari/mainnet/deployer.json --upgrade-authority ~/.config/agari/mainnet/deployer.json --program-id ~/.config/agari/devnet/agari-desk-program.json anchor/target/deploy/agari_desk.so`. A first attempt that fails on writes is resumed with `--buffer <keypair>` (the named-buffer recipe). Verify `solana program show 4gAfi… --url mainnet-beta` (authority = the deployer, 484,208 bytes) and `solana program dump … | sha256sum` against the hash above. Record `deploySignature`, `deployedSlot`, `soSha256`, `sourceCommit` under `programs.agari_desk` in `scripts/deploy/addresses.mainnet.json`.
4. **Init.** `AGARI_KEYS_DIR=~/.config/agari/mainnet pnpm deploy:init-desk --cluster mainnet-beta --dry-run` (prints the plan, the rent and whether the deployer is the upgrade authority), then without `--dry-run`: `admin_init_config` (USDC `EPjFWdd5…`, Jupiter v6 `JUP6Lkb…`, attestors = [the mainnet price-attestor], tag 101) and `public_init_reference` × 8. Re-running creates nothing. Commit `addresses.mainnet.json`.
5. **Coolify env, `agari-ops`** (commit and push first; a restart is a build): `DESK_RUNNER_PRIVATE_KEY` (the mainnet desk-runner), `PRICE_ATTESTOR_PRIVATE_KEY` (the mainnet attestor; the devnet relay's key must not be reused), `DESK_RPC_URL` (a Helius mainnet URL; the devnet `SOLANA_RPC_URL` stays what it is), `DESK_CLUSTER` unset (mainnet-beta), `JUPITER_API_KEY`, `DESK_AI_MODEL` or `AI_MODEL` with its credential, `DESK_MAX_MODEL_CALLS_PER_HOUR` (12), `OPS_ACTORS=<the current list>,desk-runner`, `DATABASE_URL` (shared), and `DRY_RUN=1` for the shadow run. **`agari-web`:** `NEXT_PUBLIC_AGARI_DESK_PROGRAM_ID=4gAfiRauANHVajpErqCey7iWGbKkyuSsnXLRbApVaWak`, `DESK_OPERATOR_ADDRESS` (the desk-runner's public key, for the studio), `SOLANA_MAINNET_RPC_URL` or the server's `HELIUS_API_KEY` for `/api/rpc/mainnet`.
6. **Shadow first.** Deploy ops with `DRY_RUN=1`: the heartbeat names the operator address and "DRY RUN"; the runner reads, values and asks the model for live desks and sends nothing. The user's desk: practice → six hourly checks → **Go live** (`ask_first`, $50–100, tiny caps) → set `DRY_RUN=0` → the first `ASKED` record, approved in the browser → the first execution (a reference post, then `operator_buy`) → `on_its_own` → the 00:05 UTC checkpoint. Evidence rows for each (plan §8 C7).
7. **What the fork taught, so C7 does not learn it again.** (a) Jupiter's `/swap-instructions` needs the quote back **whole** (the C3 client sent a zod-trimmed copy and got 422). (b) With `skipUserAccountsRpcCalls` Jupiter adds an idempotent create of the desk's ATA with the desk as payer; the client re-points it to the operator and sends it first (a no-op on mainnet where the ATA exists). (c) A route carries 28–33 accounts and the transaction is ≈ 870 B: `MAX_ROUTE_ACCOUNTS` is 40, the 1,232-byte limit is the real bound. (d) OpenAI's Jupiter route is a thin Manifest book (≈ 170–200 bps of price impact on $100–$400): at 50 bps slippage Jupiter itself refused the fill (6001), at 200 it filled, so `SLIPPAGE_BPS` is 200 (the program's 8 % band and the gate's 250 bps cost cap still bound it). (e) Whether `outAmount` nets the 100 bps transfer fee depends on the venue: Meteora DLMM nets it, Manifest quotes gross (§7). (f) A stranger's `owner_withdraw` is refused by Anchor's seeds constraint (2006) before the program's `NotOwner` can fire: the desk PDA is derived from the signer. (g) The runner never posted a desk's first reference (a never-posted `DeskRef` is all zeros and the gate denied every trade): it now posts from the feed, with the mint's own multiplier, before any action.
