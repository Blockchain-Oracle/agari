...
- **Devnet drive + browser pass (2026-09-16, stage owner; run from worktree `w1` @ `6c9ed94`, whose code matches the deployed programs):**
  - **Drive passed** on the live **TSLA-5m #73** `Da8cXT7A…` (14:15:29Z → 14:22:34Z): faucet SOL top-up `661ULnET…` → tUSDC mint `61Dx7MPF…` → **wallet-paid IOC Up fill** `3w7kDdBa…` (fee payer the user's wallet `6uXSkDbh…`, its 5,000-lamport fee out of the faucet's 0.02 SOL; 2,002 lots for 1,353,352 base at avg 6,760 bps; 17,718 CU / 586 B) → cross-check **settle** `3Z6ZfoW3…` (winner **NO**, so the Up call lost) → **claim** `5ijXByUf…` (`user_redeem`, 14,103 CU, the 250,000 seat bond returned). All rows in `acceptance.md`; every signature finalized.
  - **Both claim paths on one Window:** the settler's crank `redeem_for` `4j2ThmKv…` paid the seat maker 2,252,000 (payout 2,002,000 + bond) and `PublicCloseLedger` `4XYECc4q…` closed the Ledger. The index projects `redeemed_by_crank` **false** for the taker and **true** for the maker. Settlement reconciles exactly: 1,353,352 + 648,648 = the 2,002,000 payout.
  - **Timing, the reason attempt 1 failed** (aborted before signing anything, on #72 at 14:11:15Z): the seat maker quotes a fresh 5m Window only ≈ 2.5 min after it opens, and its orders carry `expire_ts` 44 s before lock — a ≈ 95 s fill window. The window-roller also pre-lists the next Window, so a Series' `nextIndex - 1` can name a Window that is Listed but not trading. **Gate a drive on the Book carrying live nodes with ≥ 75 s left, not on the clock.**
  - **Faucet funding for the drive (`6c9ed94` in `w1`, needs porting to this branch):** `first-call-kit`'s `newUser` used `requestAirdrop` on devnet, which the kit itself flagged as a stand-in. `openDrive` now takes `solSource`, defaulting to the `sol-faucet` role on devnet and the cluster airdrop on Surfpool. tUSDC still comes from the faucet mint authority via `fundUser`, whose fee payer is the **deployer** (it pays the 1,488,440-lamport ATA rent).
  - **Browser pass, no wallet extension** (headless CDP, own profile on :9357, against `next start` on :3000 during the open session): **signed-out**, **first-run** and **an open market with a live Window**, each at **390 / 768 / 1440 in both themes** — 18 captures, then 6 re-captures for first-run. Every capture: **0 console errors, 0 failed requests, `scrollWidth - innerWidth = 0`**. The live Window renders hero, chart with opening print, countdown, UP/DOWN prices, ticket rail with "Connect a wallet to bet", the session chip "OPEN · CLOSES IN 5H 46M", and the ticker picker. First-run shows the "Welcome to Agari" walkthrough over a blurred backdrop, 5 steps, Skip/NEXT.
    - **Gotcha:** the capture harness reuses one Chrome profile across jobs, so `agari.tutorialSeen` set by an earlier job suppressed the walkthrough; first-run jobs must `removeItem` it. The flag is `booleanCodec`, i.e. `"1"`/`"0"`, not `"true"`.
  - **Still needs the user's real wallet** (box left unticked): unfunded, quote moved (requote), fill, nothing filled, unknown send (kill the tab mid-send), win/loss/void, claim and crank-paid, and the kill switch. Closed and paused were covered by S18; the token lanes are paused on `quote-unavailable` today.
