...
| `5c8cd6a` (S18c.2) | Additive by D-085 — hero head badge (`.mh-asset-badge`, 28 px): the asset's vendored mark on its own brand disc replaces the typed monogram; Masayume's disc geometry kept, its paint cleared by `has-mark` |
| `5c8cd6a` (S18c.2) | Additive by D-085 — rail card glyph (`.mc-asset .glyph`, 26 px), reel badge (`.reel-badge`, 32 px), word-board tile (`.wq-btc`): the same mark in Masayume's three other discs; an asset the registry does not know keeps the disc with its initial |
| `5c8cd6a` (S18c.2) | Additive by D-085 — Agari's own discs (ticker picker tab, Lucky, Stage, hedge banner): the mark; a share-token lane (TSLAx, NVDAx…) wears the underlying's mark with the 8 px mono issuer badge (`data-xstock`); QQQ/VOO/SPY type their monogram on the issuer colour (no clean issuer SVG); Apple's near-black disc gets a hairline ring on the dark theme only |
| `cbbe5d2` (S18c.3) | Additive by D-085 — header marquee + `TickerItem` (`.marquee-mark`, 12 px; 10 px under 720 px): the mark leads each price cell; Masayume's brand row untouched. Verified 390/768/1440, both themes |
| `cbbe5d2` (S18c.3) | Additive by D-085 — ticker hub h1 (`.tkh-mark`, 48 px; 40 px on a phone) beside the display face; `/tickers/TSLA` verified both themes |
| `cbbe5d2` (S18c.3) | Additive by D-085 — Sensei trade cards (`.sensei-mark`, 16 px) on the card head's asset line; verified in the open drawer with injected cards (no live Windows while closed) |
| `cbbe5d2` (S18c.3) | Additive by D-085 — portfolio and profile `BetRow` (`.bets-mark`, 18 px) leading the call's link, badge on token rows; verified in `/u/<address>`'s calls plate with injected rows, both themes |
| `cbbe5d2` (S18c.3) | Additive by D-085 — share cards (`features/share/marks.ts`, 56 px): `drawAssetMark()` draws the disc, Path2D glyph or typed monogram and the token badge on the canvas beside the call and trade eyebrows, brand hex by import from `TICKERS`; the `/dev/share` call-card PNG opened at 1600×900 |
| `aa62e42` (S18c.4) | `THIRD_PARTY_NOTICES.md`: header names Agari (was "used by Masayume"); "Asset marks" section with pinned simple-icons versions (16.31.0; Amazon from 14.15.0, removed in 15.0.0 pending permission; Microsoft own geometry, simple-icons removed it in 13.0.0 on trademark terms), nominative use only |
| `bae07eb` (S18a.5) | Additive by D-086 — closed hero (`HeroAssetChart`/`HeroAssetHead`): Masayume's `.hero-chart` skeleton with an asset in it — mark + name + ticker where the Window's asset was, `HistoryRangeTabs` (1D) in the `.mh-cadence-tabs` slot, the last price in `.mh-question` with a `.pair-meta` "Last close · as of 16:00 ET" (or "Pre-market / After hours · as of" for a moved tick), the day's move in `.mh-distance` with its "since" word, the countdown in `.mh-settles`; the lazy `PriceChart` against the previous close (`lineLabel`); the foot says phrase · last close · source. Aged readings never a `StaleTick`. Verified `/dev/hero` 390/768/1440 both themes |
| `bae07eb` (S18a.5) | Additive by D-086 — ticket rail placeholder (`TicketPlaceholder`, `.tk-ticket--rail`): the ticket's own panel shell with the phrase, the meanwhile line and "Read the wire →" (`/news?symbol=`); the schedule seam renders nothing until 18f |
| `bae07eb` (S18a.5) | Additive by D-086 — closed Regular lane (`NextWindowRail`/`NextWindowCard`): `.market-card.market-card-pending[data-next]` as `PausedCard`/`GapListedCard` draw it, with the live card's price bar + `CardSpark` above the pending copy ("First 5m Window opens Tue 09:30 ET" — the hour lane says 10:00); `LaneTabs` carry configured lanes at zero (`extraKeys`); `BetweenRounds` closed = phrase + next action; word board closed = `EmptyState` + "Read the wire" |
| `bae07eb` (S18a.5) | Additive by D-087 — session chip: `data-state` is core's `SessionState`, the word from `sessionStateWord`, the tail from `sessionPhrase`, 30 s tick; pre/post lit not pulsing, holiday half-ink; mounted in `.header-right` from 1024 px (`.header-session`; D-087 said 768, but at 768 the desktop nav already fills the bar and the chip pushed Connect off the edge — found on the pass, moved to 1024), hidden below where the marquee's session cell carries it. Verified `/markets` 390 (header chip `display: none`, lanes chip carries the tail), 768 (hidden, Connect whole) and 1024/1440 (header chip live) |
| `bae07eb` (S18a.5) | Additive by D-086/D-087 — marquee: `OPENS IN · 1H 12M` / `REOPENS · TUE 09:30 ET` / `CLOSES IN · 2H 05M` session cell; a slot with no feed shows the archived close, flat, tagged `CLOSE`; `LOADING` only while nothing is known, else `NO PRICE FEED`; closed, the fallback poll slows to 60 s. Portfolio empty Open tab: phrase + "See what lists next" |
| `7c87df2` (S18a.6) | `/dev/hero` fixtures: the closed hero + rail in `.markets-hero .hero-grid-mini`, 5m and 60m next-Window cards and the chip at `CLOCK.preTue/postTue/weekendSat/holidayThu` over a canned signed-archive session; the fixture calendar starts 09-08 so a Tuesday morning has a previous close |
| S18a.7 | Found on the pass: Masayume's `.hero-chart-foot > span:first-child { max-width: 156px }` (part-16) capped the closed foot line at 156 px and wrapped it into the clipped panel — the foot line is a `div` now (Masayume's CSS untouched); `/dev/session` chips passed no clock, so the new phrase counted from the live clock ("opens in <1m") — every `SESSION_STATES` entry carries its `nowSec`. Known and left: the closed chart's time axis is the shared `PriceChart`'s UTC scale (a session spanning midnight UTC labels the day, "15"); Masayume's chart never crossed a day |
| `152a83f` (S18d.4) | Additive by D-082 — `/news` lead: Masayume's lead (index, meta, 48 px Sora headline, arrow) plus a 2 px edge in the tone's ink (`--profit`/`--loss`, hairline when neutral), the tone as a dot + word where the bordered pill was, and under the headline the stocks it names as 20 px marks (18c's `AssetDisc`) with `$TICKER` cashtags to `/tickers/[SYM]` (`Article.symbols`, shown at last). Head, JP subtitle, live dot, intro, skeleton, 60 s poll untouched. Verified 390/768/1440 both themes |
| `152a83f` (S18d.4) | Additive by D-082 — the wire row (`NewsRow`, `styles/news-wire.css`): one grammar `index · mark slot · body · tone · arrow` as a `<li>` whose title is its only link; the edge on every row; 16 px marks in the slot; cashtags after the source in the meta line (tweak 1); under 640 px the title spans to the edge and the tone + arrow drop to the meta row (tweak 2, fixed in `S18d.7`). Masayume's row values kept where they are not the grammar's (rule, hover, title face). DOM: no nested anchors; 1 link per row (+1 per cashtag) |
| `36d8c37` (S18d.5) | Additive by D-082 — activity feed (`ActivityList`): the same `NewsRow`; the wallet's hue avatar at 16 px in the mark slot (was a 10 px dot in the meta), the event kind as the tone's word in the kind's ink, the name link kept in the meta on following/ticker feeds. Verified on `/tickers/TSLA` (6 rows, both themes, 390/1440); `/activity` itself needs a wallet (Connect card, no rows to pass) |
| `36d8c37` (S18d.5) | Additive by D-082 — ticker hub headlines: `NewsRow` with each story's marks and `$TSLA` cashtags; `.tkh-headlines` takes the activity list's 24 px rhythm; the anchor-as-body override in ticker-hub.css removed. Verified `/tickers/TSLA` 390/1440 both themes |
| `3eeeba5` (S18d.6) | Additive by D-082 — take card `.take-chip`: the asset's mark (16 px) before `▲ UP`, the band's asset as a `$TSLA` link to its hub (a token links to its underlying's), the reference's words after it; caption, avatar, badge, provenance, CTA untouched. Composer "You're calling" line = the same chip (mark, direction, `$NVDA under $212.19`) with the Window after it, in the Room's ink. Verified `/reels` take card and the open composer, 1440 dark; chip DOM `take-chip-mark has-mark · dir · dot · band` |
| `b695304` (S18e.1) | Additive by D-083/D-084 — `CapabilityPending` gains a way out (`.cp-action` "Make a call on the markets →", the meta's mono voice in the accent, `:focus-visible` ring) so Earn, Parlay, Range, Moonshot and the agents desk never dead-end; their dependency lines name the Solana program and that it is planned after the hackathon deadline (no Foundry scripts in user copy); X not configured reads "X sign-in is not available on this deployment yet" (was "temporarily unavailable, try again"); the strategies nameplate reads SOLANA DEVNET. Walked at 1440 and 390: /earn /agents /strategies /parlay /trade-from-x /claim /pool /bell /beta /games ×10 /dev/x — no blank page, no infinite loading, no horizontal overflow. |
| `7c08c1c` (S18e.2) | Exact (semantics only) — nine click-to-close scrims (AddFunds, SessionModal, CopyDrawer, GameSettingsSheet, HowToSheet, DuelResultModal, LuckyResultModal, TicketDock backdrop, SenseiDrawer scrim) are `<button tabIndex={-1} aria-label=…>` with their own classes and paint (one reset in `modal.css`; the games root gains a transparent `.du-modal-scrim` under the sheet); eight inputs keep the outline off only under `:focus-visible` beside the border ring they already had (the ticket amount gains a vermilion hairline, the X card amount a focus-within border). `21st review web/src` errors 25 → 2; the two left are `components/ui/tabs.tsx` and `toast.tsx` (untouched by D-083). Keyboard at 1440: Enter on a focused card opens its Window; Shift+Tab lands on the ticker picker with the vermilion ring; the rail ticket's controls are in the tab order, the backdrop is not. |
| `d400f38` (S18e.3) | Additive by D-083 — the verdict stamp's words: 上がり "agari — it came in" for a win, 放銃 "hōjū — it dealt in" for a loss (the old brand's 正夢/逆夢 pun is gone; stamp, colour law and layout unchanged); the reel's closed card reads the session phrase through the one `sessionClosedLine`; `/how-it-works` states the Solana facts in the same sections (Wallet Standard wallets, SOL then tUSDC in one signature, nine stocks on 5m/15m/1h + Gap + token, seats on the Window's ledger, RedStone/Pyth prints, permissionless cranks); share cards name `NEXT_PUBLIC_SITE_URL`; query keys, the mark's comment, the private-desk domain and the claim receipt's brand say agari. |
| `92d02c0` (S18e.4) | Exact — six files knip and grep both called unreferenced removed (`FundingNote`, `StakeInput`, `DeskJoin`, `GapListedCard`, `LiveTicker`, `AppShell`); nothing rendered changes. |
