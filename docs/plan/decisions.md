...
- **Rule:**
  ...
  - **Why not Circle's devnet USDC:** only Circle can mint it, so Agari's faucet could not hand it out and "Get test funds" could not fund a fresh wallet in one signature (D-034). Circle's public faucet gives "20 USDC on testnet every 2 hours, per address, and per blockchain", behind reCAPTCHA, web only, Solana Devnet included (faucet.circle.com, read 2026-09-19). The faucet here gives 10,000 per claim, and the seat maker, the Range reserve (200) and the maker vault (100) need funding no 20-per-2-hours source could supply. The first wording of this reason ("small amounts") was written before that page was read; the user called it out, rightly. The app's copy now gives the reason that does not depend on Circle's limits: only Circle can mint USDC.
  - **Changing it later:** `config.collateral_mint` is set at init and every Window vault, the seat vault, the Range reserve and the maker vault are bound to it (`WrongMint`). Pointing the venue at another mint means a fresh venue: re-init every Series and re-fund every role. A mainnet venue would be initialised with Circle's USDC.
  - **The user's view (2026-09-19, second note):** not against tUSDC, but a recognised stablecoin "looks more real and mature". Open options that do not need a new venue: (a) Metaplex token metadata on the tUSDC mint (name, symbol, icon) so Phantom and explorers stop showing an unknown token — checked 2026-09-19: the metadata account `BVgYScK2…` does not exist, and the mint authority `6aRMkK3E…` is ours, so one devnet transaction adds it; (b) show the unit as "USDC" with a devnet-test mark. (b) names a mint that is not Circle's as USDC, so it stays the user's call.
