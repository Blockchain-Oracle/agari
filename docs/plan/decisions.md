...
- **Approval:** stage owner.
- **Amended 2026-09-22 (S20, D-125):** "no second venue publishes a pre-IPO mark for OpenAI" is no longer true. Pyth has published 24/7 valuation indices for OpenAI (`Equity.Index.OPENAI/USD`) and Anthropic since 2026-09-17. The token lane stays single-source all the same: the index prices the company's valuation while the lane prints the DEX token price, and the two sat about 11 % apart when measured, so an index cross-check would void every Window rather than catch a bad print. Where the venue's key is entitled to the index, the name gets a **valuation lane** of its own (`OPENAIV-60m`) that settles on Pyth directly, on the TSLA path; the hub shows both numbers side by side. The rule above is unchanged for the token lane; the README says which lane trusts what.
...
### D-125 — Pyth's pre-IPO valuation indices: a valuation lane per name, gated on a live entitlement probe, kept off the trial key's latch
- **Date / owner:** 2026-09-22 · S20 owner
- **Evidence:** Hermes lists `Equity.Index.OPENAI/USD` (`96d4bb23…c483`) and `Equity.Index.ANTHROPIC/USD` (`5da511a7…689d`), 24/7 since 2026-09-17, expo −5. The venue's trial key is entitled per feed: `Equity.US.TSLA/USD` → 200; both indices → 403 `requires access to one of the following groups: ["pyth-indices"]`; the public endpoint without a key → 401 (verified 2026-09-22 by hand, then by the S20.2 dry boot and the S20.3 dry run the same day). `boundary-cache.ts` latched `pythAuthFailed` on any 401/403 and then refused every Pyth fetch, and `spot-feed.ts` ended the stream on a 403: one request for an index with the trial key would have stopped TSLA/QQQ/VOO settlement and the spot stream. The Series PDA is `["series", ticker, cadence, basis]` and `910/3600/token` is already `OPENAI-60m`. On the plan day the OPENAI token sat about 11 % from the index.
- **Rule:**
  - **Own ticker.** A valuation lane is its own registry ticker (`OPENAIV` 930, `ANTHROPICV` 931, `kind: "valuation"`, `valuationOf`), token-only like a pre-IPO name. The index id lives in `Ticker.pythIndexFeedId`, on the pre-IPO name and on its lane, and never in `pythFeedId` nor under `tickers.<SYM>.pythFeedId` in `price-sources.json`: those are what the trial feed list, `symbolOfPythFeed`, the spot stream and the halt watch key on.
  - **Entitlement-gated listing.** One store per ops process (`entitled | denied | unknown` per index) and an hourly probe per feed, the Bearer key alone in its request. A 200 makes the feed entitled and measures its publish spacing and conf/price; a 401/403 makes it denied with the group Hermes names; a timeout or 5xx keeps the state it had, because a transient error is not an answer about entitlement and flipping a live lane would void its Window. The roller lists a Pyth version only on a usable feed (a trial feed always, an index while entitled) and otherwise reads `paused: no signed source (Pyth feed not entitled)`. The deploy script probes first and refuses to register while denied, with no `--force`: no dead lane is ever shown. A key with `pyth-indices` switches every lane on at the next probe, with no deploy.
  - **Kept off the latch.** The boundary cache refuses an unentitled index before any request leaves, never puts an index in the same request as a trial feed, and records a 403 on an index-only request in the store where a trial 403 still latches; the relay groups index slots apart; the archive and the index spot poll ask only for entitled feeds. Nothing gated can answer 403 on the trial key's path, and a test holds the latch down through an index 403.
  - **Grace 60 s and conf 200 bps until measured.** The index's publish cadence cannot be measured while the key is denied; the probe records it once it can, and the policy is tightened by a new version, never an edit.
- **User-visible:** the OpenAI and Anthropic hubs show "Pyth index" and "Token vs Pyth" only when the venue may read the index, and omit the rows otherwise with no explanation on screen; `/status` carries a "Pyth valuation index · entitlement" row that warns while not entitled and never degrades the figure; no valuation lane appears anywhere until the key is entitled.
- **Rejected:** listing the lane today on the promise of a key (a dead lane); putting the index under the pre-IPO name's `pythFeedId` (D-102's rejected placeholder in another coat, and a latch waiting to fire); making the index the token lane's cross-check (it prices the valuation, the lane prints the DEX token price, and they would diverge every Window).
- **Approval:** the user, 2026-09-22 (plan §§3, 6, 7). Whether to ask Pyth for `pyth-indices` is his call; nothing else waits on it.

### D-126 — `agari-desk`: an agent holds a basket of PreStocks for its owner, inside limits the program enforces

**Settled 2026-09-22 (the user's approval of plan `quizzical-booping-ocean` §4–§5, §8 Lane C; the earlier "agents do not trade real tokens" rule withdrawn by him the same day).** Shijima's desk (`open-serv`, Robinhood Chain) moves to Solana mainnet, PreStocks and Jupiter. Spec: `docs/plan/specs/desk.md`.

- **What the program does and does not decide.** The owner decides *what* to own (a basket preset, a cash sleeve, limits) and signs every owner call with the wallet. The operator (an ops key) decides only *when*, and the program checks every operator action in a fixed order: caps per action and per fixed 24 h window, the allowed names, a fresh venue-attested reference (≤ 900 s), a premium ceiling over the PreStocks mark, an 8 % band floor on what must come back, an exact-spend post-check, and a leak check that no token account owned by the desk PDA was slipped into the route. `operator_checkpoint` seals "did nothing" into the same hash chain, so a record with no trade is still fingerprinted on chain.
- **Jupiter by CPI, without an IDL.** The route is opaque bytes plus remaining accounts; `#[account(address = config.swap_program)]` is the whole program check, and the desk PDA is flagged as signer in the forwarded metas (`invoke_signed`). Because the route is an operator argument, the program pins the *outcome* rather than the path: spent == `usdc_in`, received ≥ `max(min_out, floor)`, nothing leaked. `swap_program` is a config field so LiteSVM tests substitute a stub router (`agari-swap-stub`) and exercise every post-check, including the misbehaving ones.
- **The reference is attested, not read from a mint.** PreStocks names have no on-chain oracle. `DeskRef ["desk-ref", mint]` is posted permissionlessly with an ed25519 signature over a 114-byte `agari-desk-ref-v1` message (program id, cluster tag, mint, token price, mark, multiplier, fetched-at), verified through the instructions sysvar exactly as attested prints are (`agari-common` `parse_ed25519_len`, of which `parse_ed25519` is the 158-byte case). The mint's ScaledUiAmount f64 is never read on the money path: the attestor signs `multiplier_e12` and every conversion goes through `VALUE_SCALE = 10^23`.
- **The trust boundary, stated.** The attestor and the operator are both ops keys today, so the band and the premium ceiling defend against a stolen *operator* key only while the attestor secret is kept apart from it (a separate mainnet `PRICE_ATTESTOR_PRIVATE_KEY` at C7). The independent leg is Pyth's `Equity.Index.<NAME>/USD` feed: an owner who sets `require_pyth_index` makes every buy carry a fully verified `PriceUpdateV2` (≤ 60 s) for the name's feed, and the premium is measured against it instead of the mark. It is switch-ready, not on, until the venue is entitled to `pyth-indices` (Lane B, D-125).
- **Practice desks are paper ledgers with no transaction.** Anyone, judges and U.S. visitors included, runs the whole pipeline on real prices and real Jupiter quotes; a "would have acted" moves a paper ledger at the quote net of PreStocks' 100 bps fee; the record is hashed with the same `desk.v1` canonical bytes, so "Check it" works before a desk has ever touched the chain. Six practice checks unlock **Go live**.
- **The mainnet browser session.** The app's read runtime stays bound to devnet (bets, cover, Trading Balance). The desk's owner calls take an explicit mainnet RPC (`/api/rpc/mainnet`, D-121's pattern) and re-wrap the connected Wallet Standard account for `solana:mainnet`; every desk surface says the network out loud. Nothing about the desk goes through `configureMarkets`.
- **Not built, on purpose:** Shijima's savings vault (no yield leg on mainnet PreStocks), its 7-day browser session key (a wallet signature is one tap), `batch` (a Solana transaction is the batch). Sells count against the caps at `max(received, oracle value)` as Shijima found necessary on 20 Sep: the oracle value is the one number the operator cannot move.
- **Approval:** the user (2026-09-22). Mainnet deploy and his own desk's funding are his spend at C7 (plan §9).
- **Evidence 2026-09-22/23.** *The fork:* the whole desk ran on a Surfpool fork of Solana mainnet on 2026-09-22 (`scripts/drive/desk-rehearsal.ts`, `ce24370`, **31/31 checks**; `acceptance.md` rows 23:49–23:54Z marked "(fork)"): `agari_desk.so` (484,208 B, sha256 `27e46960…3f19a0`) deployed for 3.37129176 SOL rent and initialised for 0.019978 SOL; a desk opened ($500 per action, $1,000 a day, 3,000 bps, on its own), eight names allowed, 1,000 USDC deposited, the AI Labs mandate signed and stored; one live wake through the runner's own functions with two buys through the Jupiter CPI (OpenAI $400 → 0.217193247 raw via Manifest, Anthropic $398.417144 → 0.378220907 raw via Meteora DLMM), each record's hash equal to the `Bought` event's `decision_hash` and the replayed head equal to the chain's; prove-limits with the real desk-runner key landed nine refusals as failed transactions (`OverPerActionCap`, `OverDailyCap`, `BelowOracleFloor`, `PremiumTooHigh`, `ShadowMode`, `IsPaused`, `UnknownAttestor`, a stranger's `owner_withdraw` at Anchor's seeds constraint, `ReferenceStale` after +20 min); then a sell of a disallowed name, a checkpoint, and `owner_withdraw(u64::MAX)` to the owner's ATAs only; 23 transactions on the desk, 8 of them the failed refusals. The fork also measured that Meteora DLMM's quote nets the 100 bps transfer fee while Manifest's is gross (§7 of the spec). *Production:* C4 and C5 went live on `useagari.xyz` on 2026-09-22 22:05–22:32Z, and the first practice desk there (owner `EfTzYtM22yPbriCaFaKEK7pMrWoEfmFoqNtp8vDELmgp`, desk `49f67e4d-dab7-4eb4-9882-2d2a2e80a511`, AI Labs 40/40/20, $1,000 paper) was woken by the runner 90 s after its creation and wrote record #1 as **DECLINED: "OpenAI is 21.3% above its mark. Your ceiling is 10.0%."** by the pre-gate alone, with no model call; record #2 failed closed when the model's answer was cut at 2,048 tokens (fixed in `1b4ca1c`: 8,192 output tokens, low reasoning effort), and #3 declined again on the hour. *Not yet:* the mainnet deploy of `4gAfiRauANHVajpErqCey7iWGbKkyuSsnXLRbApVaWak` and the user's own live desk (his spend); no real money has moved through a desk.
