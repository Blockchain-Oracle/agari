...
| **Pre-IPO valuation indices, wired and gated.** `OPENAIV-60m` and `ANTHROPICV-60m` settle on [`Equity.Index.OPENAI/USD`](https://app.pyth.com/explore/Equity.Index.OPENAI%2FUSD) and `Equity.Index.ANTHROPIC/USD`. The desk can require the index as its premium reference (`require_pyth_index`). Once the key has the `pyth-indices` group, the registration script accepts both Series and they list at the next hourly probe; no code change or program upgrade. | [`pyth-entitlement.ts`](services/ops/src/runtime/pyth-entitlement.ts), [`agari-desk`](anchor/programs/agari-desk/src/lib.rs) | — | the trial key is refused with a 403 today, so nothing lists ([D-125](docs/plan/decisions.md)) |
