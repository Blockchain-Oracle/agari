...
See [Halts and voids](/trading/halts-and-voids) for the honest labels the app uses for each of these, and every one of these transactions with its explorer link in the repository's `docs/evidence/acceptance.md`.

## Claim your result

Open <AppLink href="/portfolio">Portfolio</AppLink> when the Window shows a result. You can claim a pending payout yourself; after the grace period, the settler's automatic redemption job pays any remaining seat to its owner. History shows **Paid automatically** and the payout receipt when that happens. A manual claim also remains possible after the Window itself has been cleaned up on-chain, while the claim is still pending.
