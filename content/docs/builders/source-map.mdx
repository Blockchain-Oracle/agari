...
**Reviewed application:** `integration/w1` at `c412501`, fetched from `origin` on 23 September 2026. The docs site is a separate Git repository. Its source links may require access to Agari's private GitHub repository.
...
| Strategy subscriptions and copy | `anchor/programs/agari-strategy/`, `web/src/features/strategies/` | Devnet grant and subscription receipts; a runner must be configured to act |
...
| Basket and desk screens | `web/src/features/baskets/`, `web/src/features/desk/` | Production routes `/baskets`, `/desk`, `/desk/new` were present at review |
