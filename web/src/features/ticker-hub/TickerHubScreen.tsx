...
function NameFacts({ symbol, preIpo }: { symbol: TickerSymbol; preIpo: boolean }) {
...
export function TickerHubScreen({ symbol }: { symbol: TickerSymbol }) {
  ...
  const intro = basket ? TICKER_HUB.basket.intro(ticker.name, basket.members.map((m) => TICKERS[m.symbol].name).join(", ")) : preIpo ? TICKER_HUB.preIpo.intro(ticker.name) : TICKER_HUB.intro(ticker.name);
  ...
  return (
    <div className="container news-page prf-page tkh-page">
      <div className="news-inner">
        ...
        {basket ? <BasketHub basket={basket} /> : <NameFacts symbol={symbol} preIpo={preIpo} />}
