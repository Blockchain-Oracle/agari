...
describe("dayChange", () => {
  it("measures a moved pre-market tick from the last close", () => {
    ...
    expect(formatDayChange(change!)).toEqual({ dollars: "−$1.97", percent: "−0.5%", direction: "down" });
  });
  it("measures the archived close, or a feed that repeats it overnight, from the previous close", () => {
    ...
    expect(formatDayChange(dayChange(usd(359n, 7n), CLOSE_SEC, closes)!)).toEqual({ dollars: "−$3.12", percent: "−0.8%", direction: "down" });
  });
  ...
  it("formats up and flat moves with their sign, dollars at the line's scale", () => {
    ...
    expect(formatDayChange(up)).toEqual({ dollars: "+$3.60", percent: "+1.0%", direction: "up" });
    const flat = dayChange(usd(362n, 19n), CLOSE_SEC, closes)!;
    expect(formatDayChange(flat)).toEqual({ dollars: "$0.00", percent: "0.0%", direction: "flat" });
    const big = dayChange(usd(150_000n, 0n), CLOSE_SEC + 60, { last: { sec: CLOSE_SEC, priceRaw: usd(100_000n, 0n) }, prev: null })!;
    expect(formatDayChange(big)).toEqual({ dollars: "+$50,000", percent: "+50.0%", direction: "up" });
  });
});
