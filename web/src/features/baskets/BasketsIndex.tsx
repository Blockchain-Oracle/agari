...
function LiveBasketCard({ symbol, laneSet, nowMs, facts, held }: LiveCardProps) {
        ...
        <section aria-label={BASKETS_COPY.title}>
          ...
          <div className="bk-grid">
            {BASKET_SYMBOLS.map((symbol) => (
              <LiveBasketCard key={symbol} symbol={symbol} laneSet={lanes.laneSet} nowMs={nowMs} facts={facts?.ok ? (facts.value[symbol] ?? null) : null} held={held} />
            ))}
          </div>
        </section>
